...
effectively finds all assemblies or parent items that contain `node_id`.
...
```
    ...
    NodeView(('PUMP_RV1', 'VANE_ASM1', 'MOTOR_A1', 'BEARING_01', 'OIL_SYSTEM1', 'CASE_STD1', 'VANE_001'))
...
pd.DataFrame([{col:1 for col in get_all_successors(G, root)} for root in root_nodes], index=root_nodes, dtype=pd.SparseDtype("int", fill_value=np.nan))
```
...
``` python
[pd.DataFrame({root: get_all_successors(G, root)}).stack() for root in root_nodes]
```
...
``` python
get_all_successor_edges(G, 'PUMP_RV1', attrs=['quantity', 'missing_attr'], default='test')
```
...
```
    ...
    [{'source': 'BEARING_01', 'from': 'VANE_ASM1', 'to': 'BEARING_01'},
     {'source': 'BEARING_01', 'from': 'VANE_ASM2', 'to': 'BEARING_01'},
     {'source': 'BEARING_01', 'from': 'PUMP_RV1', 'to': 'VANE_ASM1'},
     {'source': 'BEARING_01', 'from': 'PUMP_RV2', 'to': 'VANE_ASM2'}]

``` python
dfs = [pd.DataFrame(get_all_successor_edges(G, root, attrs=['quantity'])) for root in get_all_roots(G)]
dfs[:2]
```
...
<tr>
<th data-quarto-table-cell-role="th"></th>
<th colspan="16" data-quarto-table-cell-role="th"
data-halign="left">quantity</th>
</tr>
<tr>
<th data-quarto-table-cell-role="th">child</th>
<th data-quarto-table-cell-role="th">BEARING_01</th>
